...
class FullPipelineGoldenExtractor:
    ...
    def extract(self, pcm: np.ndarray) -> np.ndarray:
        ...
        for f in range(n_frames):
            ...
            if not self.vad.is_voiced(power):
                continue
            
            mel_e    = b._filterbank(power)
            log_pre  = b._log_compress(mel_e)
            feat = (b._bfp_compensate(log_pre, bfpexp)
                    if self.bfp_compensate else log_pre)
            voiced_frames.append(feat)

        if not voiced_frames:
            return np.zeros((N_MELS, 0), dtype=np.uint16)
        return np.stack(voiced_frames, axis=1)
...
if __name__ == "__main__":
    ...
    print()
    print("=== VAD spectral test ===")
    silence = np.zeros(SAMPLE_RATE // 2, dtype=np.int32)
    chirp_half = (np.sin(2 * np.pi * (200 * t[:SAMPLE_RATE//2] +
                  (7000-200)/2 * t[:SAMPLE_RATE//2]**2)) * SAMPLE_MAX
                 ).astype(np.int32)
    mixed = np.concatenate([silence, chirp_half])

    full_no_vad = FullPipelineGoldenExtractor(vad_threshold=None)
    full_vad    = FullPipelineGoldenExtractor(vad_threshold=1000)

    feats_all = full_no_vad.extract(mixed)
    feats_vad = full_vad.extract(mixed)
    print("  Without VAD: %d frames" % feats_all.shape[1])
    print("  With VAD:    %d frames (threshold=1000)" % feats_vad.shape[1])
    print("  Frames dropped: %d" % (feats_all.shape[1] - feats_vad.shape[1]))
