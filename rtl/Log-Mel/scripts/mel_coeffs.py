...
# save hex files to logmel/rtl/data directory
...
# find sparse ranges per mel filter (this is for daving memory, ignore weights == 0)
start_bins = []
end_bins = []
all_coeffs = []
...
with open(os.path.join(DATA_DIR, "mel_coeffs.hex"), "w") as f:
    for m in range(N_MELS):
        for k in range(max_coeffs):
            f.write(f"{coeffs_padded[m, k] & 0xFFFF:04x}\n")

# mel_starts.hex — start bin index per mel filter
with open(os.path.join(DATA_DIR, "mel_starts.hex"), "w") as f:
    for s in start_bins:
        f.write(f"{s:02x}\n")

# mel_ends.hex — end bin index per mel filter
with open(os.path.join(DATA_DIR, "mel_ends.hex"), "w") as f:
    for e in end_bins:
        f.write(f"{e:02x}\n")

print("\nWrote files: mel_coeffs.hex, mel_starts.hex, mel_ends.hex")
...
with open(os.path.join(DATA_DIR, "log2_lut.hex"), "w") as f:
