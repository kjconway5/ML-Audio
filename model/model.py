...
class Depthwise_Piecewise_Conv(nn.Module): 
    ...
    def __init__(
        ...
        self.pointwise = nn.Conv2d(
            ...
            out_channels,
            kernel_size=(1, 1),
            stride=(1, 1),  # Pointwise conv typically has stride=1
            padding=(0, 0),
            groups=1,  # Pointwise conv should NOT be grouped
            bias=False
        )
...
class DSCNN(nn.Module): 
    ...
    def __init__(
        self, 
        n_classes: int = 1,         #number of keywords wanted for detection 
        n_mels: int = 40, 
...
def create_model(config: dict) -> DSCNN:
    ...
    return DSCNN(
        n_classes=model_cfg.get("n_classes", 6),
