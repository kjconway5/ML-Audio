...
class Depthwise_Piecewise_Conv(nn.Module):
    ...
    def __init__(
        ...
        self.depthwise = nn.Conv2d(
            ...
            stride=stride,
            padding=padding,
            groups=1,  # NNgen compatible (not true depthwise)
            bias=False
        )
    ...
    def fuse_model(self):
        torch.quantization.fuse_modules(
            self,
            [['depthwise', 'bn1', 'relu1'], ['pointwise', 'bn2', 'relu2']],
            inplace=True
        )
...
class DSCNN(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Conv2d(ds_filters, n_classes, kernel_size=1, bias=True)
    ...
    def fuse_model(self):
        # Fuse first conv block
        torch.quantization.fuse_modules(
            self,
            ['first_conv', 'first_bn', 'first_relu'],
            inplace=True
        )
        # Fuse each DS block
        for ds_block in self.ds_blocks:
...
def create_model(config: dict) -> DSCNN:
    ...
    first_conv_filters = first_conv_cfg.get("filters", 24)
    ...
    return DSCNN(
        ...
        ds_filters=ds_cfg.get("filters", 24),
